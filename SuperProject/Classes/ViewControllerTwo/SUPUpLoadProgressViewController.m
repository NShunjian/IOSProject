//
//  SUPUpLoadProgressViewController.m
//  SuperProject
//
...
@interface SUPUpLoadProgressViewController ()<SUPElementsFlowLayoutDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, TZImagePickerControllerDelegate>
/** <#digest#> */
...
@implementation SUPUpLoadProgressViewController
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerNib:[UINib nibWithNibName:NSStringFromClass([SUPUpLoadImageCell class]) bundle:[NSBundle mainBundle]] forCellWithReuseIdentifier:NSStringFromClass([SUPUpLoadImageCell class])];
}


- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    SUPUpLoadImageCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([SUPUpLoadImageCell class]) forIndexPath:indexPath];
    SUPWeakSelf(self);
    if (indexPath.item == self.selectedImages.count) {
        cell.photoImage = nil;
        cell.addPhotoClick = ^(SUPUpLoadImageCell *uploadImageCell) {
...
#pragma mark - 五类个性化设置，这些参数都可以不传，此时会走默认设置
    ...
    // 设置竖屏下的裁剪尺寸
    NSInteger left = 30;
    NSInteger widthHeight = self.view.SUP_width - 2 * left;
    NSInteger top = (self.view.SUP_height - widthHeight) / 2;
...
- (void)rightButtonEvent:(UIButton *)sender navigationBar:(SUPNavigationBar *)navigationBar
{
    ...
    [self.selectedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [[SUPRequestManager sharedManager] upload:[SUPXMGBaseUrl stringByAppendingPathComponent:@"upload"] parameters:@{@"username" : @"NJHu"} formDataBlock:^(id<AFMultipartFormData> formData) {
            ...
            SUPUpLoadImageCell *cell = (SUPUpLoadImageCell *)[self.collectionView  cellForItemAtIndexPath:[NSIndexPath indexPathForRow:[self.selectedImages indexOfObject:obj] inSection:0]];
            cell.uploadProgress = (CGFloat)progress.completedUnitCount / progress.totalUnitCount;;
            
        } completion:^(SUPBaseResponse *response) {
            if (response.error) {
...
{
    return [[SUPElementsFlowLayout alloc] initWithDelegate:self];
}
