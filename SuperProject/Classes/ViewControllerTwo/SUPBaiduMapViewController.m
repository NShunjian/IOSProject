...
#import "SUPBaiduPointAnnotation.h"
#import "SUPCoordinateModel.h"
#import "SUPAnnotationCustomPopView.h"
#include <stdio.h>
#include <malloc/malloc.h>
#import "MyLocation.h"
@interface SUPBaiduMapViewController ()<BMKGeneralDelegate, BMKMapViewDelegate,BMKLocationManagerDelegate,BMKLocationServiceDelegate, BMKRouteSearchDelegate,BMKPoiSearchDelegate,CLLocationManagerDelegate>{
...
/** <#digest#> */
@property (nonatomic, strong) NSMutableArray<SUPCoordinateModel *> *coordinates;
...
@implementation SUPBaiduMapViewController
...
- (void)viewDidLoad {
    ...
    //    BOOL ret = [self.mapManager start:SUPThirdSDKBaiduMapKey  generalDelegate:weakself];
    //    if (!ret) {
    //
...
-(void)initBlock
{
    __weak SUPBaiduMapViewController *weakSelf = self;
    self.completionBlock = ^(BMKLocation *location, BMKLocationNetworkState state, NSError *error)
    {
        ...
        SUPBaiduMapViewController *strongSelf = weakSelf;
...
{
    [self.coordinates enumerateObjectsUsingBlock:^(SUPCoordinateModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        SUPBaiduPointAnnotation *annotation = [[SUPBaiduPointAnnotation alloc] init];
...
- (BMKAnnotationView *)mapView:(BMKMapView *)mapView viewForAnnotation:(id <BMKAnnotation>)annotation
{
    ...
    if ([annotation isKindOfClass:[SUPBaiduPointAnnotation class]]) {
        
        SUPBaiduPointAnnotation *myAnnotation = (SUPBaiduPointAnnotation *)annotation;
        ...
        switch (type) {
            case 0:
            {
                ...
                if (!pinAnnotationView) {
                    ...
                    pinAnnotationView.centerOffset = CGPointMake(0, -pinAnnotationView.SUP_height * 0.5);
            ...
            default:
            {
                ...
                SUPCoordinateModel *coordinateModel = self.coordinates[selectedIndex];
                
                SUPAnnotationCustomPopView *popView = [SUPAnnotationCustomPopView popView];
...
- (void)gotoThisPlaceAndLocation:(UIButton *)btn
{
    ...
    SUPCoordinateModel *endModel = self.coordinates[btn.tag];
...
- (NSMutableArray<SUPCoordinateModel *> *)coordinates
{
    if(_coordinates == nil)
    {
        _coordinates = [NSMutableArray array];
        
        SUPCoordinateModel *coordinate = [[SUPCoordinateModel alloc] init];
        ...
        SUPCoordinateModel *coordinate1 = [[SUPCoordinateModel alloc] init];
