...
- (void)setDimBackground:(BOOL)needDidBackground {
    _dimBackground = needDidBackground;
    
    self.backgroundColor = _dimBackground ? RGBA(0, 0, 0, 0.3) : [UIColor clearColor];
}
...
- (void)layoutSubviews {
	...
	CGFloat maxWidth = bounds.size.width - 2 * margin - 2 * outsideMargin;
    ...
    CGFloat remainingHeight = bounds.size.height - totalSize.height - kPadding - 2 * margin - 2 * outsideMargin;
	...
	remainingHeight = bounds.size.height - totalSize.height - kPadding - 2 * margin - 2 * outsideMargin;
	...
	if (square) {
		...
		if (max <= bounds.size.width - 2 * outsideMargin) {
			totalSize.width = max;
		}
		if (max <= bounds.size.height - 2 * outsideMargin) {
...
//	} else {
        CGContextSetFillColorWithColor(context, RGBA(0, 0, 0, self.opacity).CGColor);
//	}
...
- (void)updateUIForKeypath:(NSString *)keyPath {
	if ([keyPath isEqualToString:@"mode"] || [keyPath isEqualToString:@"customView"] ||
		...
		if ([indicator respondsToSelector:@selector(setProgress:)]) {
			[(NSObject *)indicator setValue:@(progress) forKey:@"progress"];
		}
		return;
	}
	[self setNeedsLayout];
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		_progress = 0.f;
		_annular = NO;
        _progressTintColor = RGB(35, 205, 119);
        _backgroundTintColor = [UIColor grayColor];
		[self registerForKVO];
	}
	return self;
}
