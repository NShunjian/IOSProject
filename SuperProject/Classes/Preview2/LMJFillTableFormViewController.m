...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    NSLog(@"%@", current);
    LMJWordItem *item = self.sections[self.section].items[textField.tag - 100];
    item.subTitle = current;
    
    LMJSettingCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:textField.tag - 100 inSection:self.section]];
