...
#import "SUPRunTimeTest+SUPMethod.h"
#import "SUPRunTimeTest+SUPWork.h"
//#import "UIAlertView+SUPBlock.h"

@interface SUPRunTimeViewController ()
/** <#digest#> */
@property (nonatomic, strong) SUPRunTimeTest *myRunTimeTest;
...
@implementation SUPRunTimeViewController
...
- (void)viewDidLoad {
    ...
    self.myRunTimeTest=[[SUPRunTimeTest alloc] init];
    ...
    [self.myButton setTitle:@"方法替换或者交换(UIButton+SUPBlock)" forState:UIControlStateNormal];
...
- (void)rightButtonEvent:(UIButton *)sender navigationBar:(SUPNavigationBar *)navigationBar
{
    ...
    [sender addActionHandler:^(NSInteger tag) {
        ...
        [UIAlertController mj_showAlertWithTitle:@"提示" message:@"跟按钮添加了 actoinblock" appearanceProcess:^(JXTAlertController * _Nonnull alertMaker) {
            ...
            alertMaker.addActionDefaultTitle(@"确认看 UIButto+SUP");
...
-(void)getClassMemberVariable
{
    ...
    //动态修改变量的值
    SUPRunTimeTest *testModel=[[SUPRunTimeTest alloc]init];
    testModel.name=@"njhu";
...
-(void)getClassProtocol
{
    ...
    NSLog(@"SUPRunTimeTest is%@ responsed to protocol %s", class_conformsToProtocol([self.myRunTimeTest class], protocol) ? @"" : @" not", protocol_getName(protocol));
    
    //******显示内容如下******
    //SUPRunTimeTest is responsed to protocol NSCoding
}
...
{
    SUPRunTimeTest *test=[[SUPRunTimeTest alloc]init];
    [test setWorkName:@"XM"];
