...
@implementation SUPUMengHelper
+ (void)UMAnalyticStart {
    
    // 友盟统计
    // UM统计
    UMConfigInstance.appKey = SUPThirdSDKUMConfigInstanceAppKey;
    //    UMConfigInstance.ChannelId:@"Web" 中的Web 替换为您应用的推广渠道。channelId为nil或@""时，默认会被当作@"App Store"渠道。
    UMConfigInstance.channelId = SUPThirdSDKUMConfigInstanceChannelId;
...
+ (void)UMSocialStart
{
    ...
    //设置友盟appkey
    [[UMSocialManager defaultManager] setUmSocialAppkey:SUPThirdSDKUMSocialAppkey];
    
    // 获取友盟social版本号
    //NSLog(@"UMeng social version: %@", [UMSocialGlobal umSocialSDKVersion]);
    
    //设置微信的appKey和appSecret
    [[UMSocialManager defaultManager] setPlaform:UMSocialPlatformType_WechatSession appKey:SUPThirdSDKWeChatAppKey appSecret:SUPThirdSDKWeChatAppSecret redirectURL:SUPThirdSDKWeChatRedirectURL];
     ...
     */
    [[UMSocialManager defaultManager] setPlaform:UMSocialPlatformType_QQ appKey:SUPThirdSDKQQAppKey  appSecret:SUPThirdSDKQQAppSecret redirectURL:SUPThirdSDKQQRedirectURL];
    
    //设置新浪的appKey和appSecret
    [[UMSocialManager defaultManager] setPlaform:UMSocialPlatformType_Sina appKey:SUPThirdSDKSinaAppKey  appSecret:SUPThirdSDKSinaAppSecret redirectURL:SUPThirdSDKSinaRedirectURL];
...
+ (void)UMPushStart:(NSDictionary *)launchOptions
{
    ...
    [UMessage startWithAppkey:SUPThirdSDKUMessageStartWithAppkey launchOptions:launchOptions];
    ...
    center.delegate = (SUPAppDelegate *)[UIApplication sharedApplication].delegate;
...
//        [self shareTextToPlatformType:platformType];//因为调用了self 所以会报错  //必须官网注册
...
//    //就这句话,重点
//    //设置文本
//    messageObject.text = [NSString stringWithFormat:@"%@%@",title,shareURL];
//
//    //创建图片内容对象
//    UMShareImageObject *shareObject1 = [[UMShareImageObject alloc] init];
//
//    shareObject1.shareImage = thumbImage;
//
//    //分享消息对象设置分享内容对象
    ...
    [[UMSocialManager defaultManager] shareToPlatform:platformType messageObject:messageObject currentViewController:nil completion:^(id data, NSError *error) {
        if (error) {
            ...
            if ([data isKindOfClass:[UMSocialShareResponse class]]) {
                ...
                //分享结果消息
                NSLog(@"分享结果消息===response message is %@",resp.message);
                //第三方原始返回的数据
                NSLog(@"第三方原始返回的数据===response originalResponse data is %@",resp.originalResponse);
            ...
            // 分享成功的通知
            [[NSNotificationCenter defaultCenter] postNotificationName:SUPUMSocialShareSucceedNotification object:shareURL];
        }
