...
@interface SUPWebViewController ()
...
@implementation SUPWebViewController
...
- (void)viewDidLoad {
    ...
    SUPWeakSelf(self);
    [self.webView addObserverBlockForKeyPath:SUPKeyPath(weakself.webView, estimatedProgress) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
    ...
    if ([self webViewController:self webViewIsNeedAutoTitle:self.webView]) {
        ...
        [self.webView addObserverBlockForKeyPath:SUPKeyPath(self.webView, title) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
    ...
    [self.webView.scrollView addObserverBlockForKeyPath:SUPKeyPath(self.webView.scrollView, contentSize) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
...
#pragma mark - SUPWebViewControllerDataSource
// 默认需要, 是否需要进度条
...
//        webView.scrollView.delegate = self;
        ...
        if ([self.parentViewController isKindOfClass:[UINavigationController class]]) {
            ...
            if ([self respondsToSelector:@selector(SUPNavigationHeight:)]) {
                ...
                webView.scrollView.contentInset = UIEdgeInsetsMake([self SUPNavigationHeight:nil], 0, 0, 0);
                // AppDelegate 进行全局设置
...
- (UIProgressView *)progressView
{
    if(_progressView == nil && [self.parentViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        progressView.SUP_height = 1;
        
        progressView.SUP_width = kScreenWidth;
        
        progressView.SUP_y = self.SUP_navgationBar.SUP_height;
...
- (void)dealloc
{
    NSLog(@"SUPWebViewController -- dealloc");
